...
var carajo = @"SELECT  cargo.ejercicio, cargo.tipo, cargo.idperiodoacademico, cargo.idestudiante, est.idtestudiante, cargo.periodo, cargo.idmoneda, tgm.idtmoneda, sum(cargo.importemd) as importemd
                        ...
                        ,nactual.grado grado_actual, nactual.nivel as curso_actual    ,nsiguiente.grado grado_siguiente, nsiguiente.nivel as curso_siguiente                    
                            from tbl_aca_estudiantecargo cargo
	                            ...
	                            inner join tbl_aca_academianivel nactual on nactual.idacademianivel = matri.idacademianivel 
	                            inner join tbl_aca_academiaarea areaactual on areaactual.idacademiaarea = nactual.idacademiaarea
	                            where idestudiante = '" + this.IdTEstudiante + @"' and per.idperiodoacademico = (SELECT idperiodoacademico from tbl_aca_periodoacademico where idtperiodoacademico = " + this.Ejercicio + @"))
	                            ...
	                            inner join tbl_aca_academianivel nactual on nactual.idacademianivel = matri.idacademianivel 
	                            inner join tbl_aca_academiaarea areaactual on areaactual.idacademiaarea = nactual.idacademiaarea
	                            where idestudiante = '" + this.IdTEstudiante + @"' and per.idperiodoacademico = (SELECT idperiodoacademico from tbl_aca_periodoacademico where idtperiodoacademico = " + (this.Ejercicio + 1) + @"))
